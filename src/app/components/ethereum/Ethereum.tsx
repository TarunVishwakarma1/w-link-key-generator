...
import { generateEthereumWallet, encryptValue } from './ethereumKeyGen/EthKeyCommon';
...
import EthereumImage from '../ui/assets/images/ethereum-eth-logo.svg';
...
const Ethereum: React.FC = () => {
  ...
  const [keyValues, setKeyValues] = useState<EthereumWallet | null>(null);
  const [wallets, setWallets] = useState<EthereumWallet[]>([]);
  ...
  useEffect(() => {
    const storedWallets = localStorage.getItem(ETHEREUM_STORAGE_KEY);
    ...
    if (storedWallets && password) {
      decryptWallets(storedWallets, password)
        .then((decryptedWallets) => {
          const ethereumWallets = JSON.parse(decryptedWallets).filter((wallet: EthereumWallet) => wallet.type === 'Ethereum');
  ...
  const handleKeyGeneration = async () => {
    ...
    const keys = generateEthereumWallet({ state: state, mnemonic: ethValue });
    setState((prevState) => prevState + 1);

    if (keys) {
      const newWallet: EthereumWallet = {
        type: 'Ethereum',  // Specify the type as 'ethereum'
        walletName,
        ...
        if (password) {
          encryptWallets(JSON.stringify(updatedWallets), password)
            .then((encryptedWallets) => {
              localStorage.setItem(ETHEREUM_STORAGE_KEY, encryptedWallets);
            })
            .catch((error) => {
      ...
      setKeyValues(newWallet);  // Set the full EthereumWallet object
  ...
  const handleDelete = () => {
    ...
    if (deleteIndex !== null && password) {
      decryptWallets(localStorage.getItem(ETHEREUM_STORAGE_KEY)!, password)
        .then((decryptedWallets) => {
  ...
  const handleDownloadWallet = async (wallet: EthereumWallet) => {
    const password = localStorage.getItem(PASSWORD_STORAGE_KEY);
    if (!password) {
